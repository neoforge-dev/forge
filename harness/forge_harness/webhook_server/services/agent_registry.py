...
Manages active agent sessions and their state.
Provides file-based persistence for restart resilience.
"""
...
DEFAULT_PERSISTENCE_FILE = Path(".forge/agent_registry.json")
...
class AgentRegistry:
    ...
    def __init__(self, expiry_seconds: int = 300, persistence_file: Path | None = None):
        """Initialize registry.

        Args:
            expiry_seconds: Seconds of inactivity before agent expires (default 5 min)
            persistence_file: Path to JSON file for persistence (default: .forge/agent_registry.json)
        """
        self._agents: dict[str, AgentSession] = {}
        self._lock = Lock()
        self._expiry_seconds = expiry_seconds
        self._persistence_file = persistence_file or DEFAULT_PERSISTENCE_FILE
        self._load_from_file()

    def _load_from_file(self) -> None:
        """Load agents from persistence file if it exists."""
        ...
        try:
            ...
            for agent_data in data.get("agents", []):
                # Reconstruct AgentSession from dict
                agent = AgentSession(
                    id=agent_data["id"],
            ...
            logger.info(f"Loaded {len(self._agents)} agents from {self._persistence_file}")
        except Exception as e:
            logger.warning(f"Failed to load agent registry: {e}")

    def _save_to_file(self) -> None:
        """Save agents to persistence file."""
        try:
