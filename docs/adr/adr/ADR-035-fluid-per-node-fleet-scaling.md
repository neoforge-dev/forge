...
**Status:** Proposed
**Date:** 2026-03-07
**Author:** node-1 lead + 3-agent council (architect-advisor, patterns-researcher, backend-engineer)
...
ADR-034 Phase 1 shipped a daemon patrol (`fleetScaleRecommendPatrol`) on node-1 that writes
...
| 4 | Per-node daemon rejected ("complexity; node-1 as hub") | Per-node **scaling authority** accepted; per-node **task queue** rejected | Queue correctness requires centralized SQLite (exactly-once claim). Scaling correctness only requires local RAM + token data. These are separable concerns. |
...
```
Task Queue (node-1 SQLite) — stays centralized, ensures exactly-once claim
     ↑ HTTP claim (FORGE_API_URL=http://node-1:8081) from all nodes

Node-Local Scaling (each node daemon) — independent inflate/deflate decisions
     reads: local /proc/meminfo + local token budget JSON + queue depth from node-1 API
...
**Why not hub-only (current):** node-1 (16GB, 4 cores) cannot SSH to other nodes to spawn agents
...
The only external dependency is queue depth, which is a single GET read from node-1's API.
...
other nodes call `GET /api/token-budgets?node=node-2` and read from the SQLite snapshot.
...
3. agent has no assigned or running task in node-1 queue
...
var ForbiddenAgentTypes = map[string][]string{
    "node-1": {"opencode", "kilo"},
    "node-4": {"opencode", "kilo"},
    "node-5": {"opencode", "kilo"},
}
```
...
- **node-1:** kimi (100MB — cheapest option; frees node-1's 16GB headroom for orchestrator)
- **node-2:** gemini (300MB — fast, reliable, no compound token limits)
- **node-3:** gemini or kimi
- **node-4/node-5:** kimi
...
```
AGENT INVENTORY — node: node-1 — 2026-03-07 14:30 UTC
...
| Seed token budgets | `.forge/heartbeat/token-budgets-node-1.json` | Write initial state (anthropic 18% weekly, openai 29% weekly) |
...
| Deploy on node-2 | node-2 has 64GB — safest first node for autonomous scaling |
...
|---------|--------|-----------|
| node-1 outage | No new task claims; running agents continue | Accept — running tasks not lost; agents retry claim every 30s. Future: litestream SQLite replication to node-2 (not now). |
...
|---|----------|---------|------|
| Q1 | Daemon deployment priority | **node-2 first** — 64GB safe testbed; precondition: node-1 Phase 1 gates complete | 3/3 |
| Q2 | Seed token budgets now? | **YES** — seed with live data; openai `COOLDOWN until 2026-04-01` immediately | 3/3 |
| Q3 | node-1 ceiling: 2 or 3? | **Keep at 2** — preserves medium-tier slot; OOM on node-1 kills Command Center | 2/3 |
...
| Q8 | Medium-tier auto-approve on node-2? | **Node-differentiated** — auto on node-2 (64GB), manual on node-1/node-4/node-5 | 3/3 |
...
`ForbiddenAgentTypes` lives in the daemon binary, but the task queue (on node-1) doesn't know
about it. The orchestrator may route an `opencode` task to a queue where node-1 is the only
...
| Token budget JSON seeded (node-1) | `7972ed40` | ✅ openai COOLDOWN, kimi CAUTION |
