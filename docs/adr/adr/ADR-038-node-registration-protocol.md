...
**Status:** Proposed
**Date:** 2026-03-08
**Author:** Lead Orchestrator (node-1)
**Supersedes:** None
...
The FORGE fleet discovered three compounding bugs that caused nodes (node-4, node-3, node-5) to be invisible in the fleet dashboard:

1. **main.go:1216 hardcoded `nodeID = "node-2"`** — any node running without `NODE_ID` env var registered as node-2, silently overwriting node-2's record.
...
- No more silent identity collisions (was `node-2` everywhere)
...
- **ADR-039**: Automatic node heartbeat (daemon sends periodic heartbeat to own DB + to node-1)
