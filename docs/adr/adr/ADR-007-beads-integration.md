...
| "Offline-first" on all nodes | **Only 2 of 4 nodes** can safely run Dolt (node-2 64GB, node-3 48GB). node-1/node-4 at 16GB are non-starters. | gemini: RAM constraints from CLAUDE.md |
...
1. **Dolt only safe on 2/4 nodes** — node-1 and node-4 (16GB) cannot run Dolt server
