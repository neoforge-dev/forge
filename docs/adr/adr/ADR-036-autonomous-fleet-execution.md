...
**Date:** 2026-03-07
**Author:** node-1 lead + council (gemini/kimi/pi — results pending)
...
FORGE_API_URL=http://node-1:8081
```
...
FLEET PLAN — node-1 — next execution in ~3min 47s
...
RESOURCE HEADROOM (live):
  Available RAM: 833MB
  Ceiling: 2/2 agents (at ceiling — no inflate possible on node-1)
  node-2: 32GB free, 3/6 ceiling (inflate possible)
...
`/proc/meminfo` validates on Linux before Darwin nodes (node-3/node-4).
Precondition checklist for any new node daemon: token-budgets-{node}.json seeded,
ceiling enforced, node-1 API reachable over Tailscale.
→ **Integrated: Phase 3 targets Linux nodes only; Darwin shim deferred to Phase 4.**

**gemini — Capability-Aware Routing:**
The orchestrator placing tasks in the queue shouldn't route `opencode` tasks to node-1
...
**codex — node-1 floor agent:**
Keep one lightweight floor agent alive on node-1 continuously. NOT overnight scale-to-zero
on the lead node — the cost of one kimi-class agent (100MB) is trivial vs cold-start delay
and broken overnight work.
→ **Integrated: min_floor applies to lead node (node-1) even during 00:00-06:00 window.**
