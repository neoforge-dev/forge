...
- The control plane config (node-1 role) — not distinguished in code from worker role
...
$ forge init --agent
  ...
  ? Control plane [http://node-1:8081]: 
...
| `bin/forge-v3-deploy-node-3` | Keep short-term | Replace with `forge node deploy node-3` |
