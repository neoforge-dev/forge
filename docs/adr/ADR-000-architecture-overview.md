...
```bash
# Via PRYA
forge dispatch send --node sati forge:kimi "Production task"
# → PRYA routes to SATI
# → Global visibility
...
```bash
# File-based protocol
.forge/xnode/lead-outbox/prya.jsonl
...
forge --node prya dispatch send forge:sati "Task"
...
*Architecture defined by sati (Node Orchestrator)*  
