...
|------|--------|----------|---------|
| sati | forge-v3 (PID xxx) | .forge/forge-v3.db | kimi, gemini, opencode |
| nova | forge-v3 (PID xxx) | .forge/forge-v3.db | claude, cursor |
| prya | forge-v3 (PID xxx) | .forge/forge-v3.db | orchestrator, minimax |
| gaea | forge-v3 (PID xxx) | .forge/forge-v3.db | claude |
...
.forge/xnode/
├── lead-inbox/
│   ├── sati.jsonl    ← Messages FROM sati (read by all nodes)
│   ├── nova.jsonl    ← Messages FROM nova
│   └── prya.jsonl    ← Messages FROM prya
└── lead-outbox/
    ├── sati.jsonl    ← Messages TO sati
    └── nova.jsonl    ← Messages TO nova
```
...
```bash
# On each node (sati, nova, prya, gaea)
cd /home/bogdan/FORGE/cmd/forge-v3
go build -o forge-v3 .
./forge-v3 --node sati --port 8081 &
...
WS_URL = os.environ.get("FORGE_WS_URL", "ws://prya:8082/ws")
