...
1. **Command Center (Python/React)**: A FastAPI `webhook_server` with 50+ API modules, a React PWA frontend (~15K LOC), SSE streaming, and a Python `notification_harness`. Deployed on prya as a uvicorn process.
