...
**Status: Accepted (Partial)** — local metrics + events real (S78d); cross-node push needs FORGE_LEAD_URL on sati/nova (human gate GATE-2/3); `forge fleet metrics` CLI pending
