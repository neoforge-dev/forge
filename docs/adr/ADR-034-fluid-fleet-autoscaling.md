...
**Status:** Proposed
**Date:** 2026-03-07
**Author:** Lead Orchestrator (prya) + Council (backend-engineer, industry-patterns, token-inventory, edge-cases)

---

## Context

FORGE currently requires manual agent spawn/kill decisions by the human orchestrator. As the task queue grows and spans multiple nodes (prya, sati, nova, vega, gaea), we need the node lead to dynamically right-size its fleet based on:

1. **Queue depth** — pending tasks vs idle agents
2. **Node RAM budget** — avoid OOM (prya at 16 GB is tight)
...
The current state: human orchestrator on prya manually runs `kimi -y` or `claude --dangerously-skip-permissions` in a tmux window. No automation. No token tracking. No cooldown awareness.
...
4. **prya already has 7 active agents despite a 2-agent documented max** — enforce hard ceilings in code first before adding dynamic scaling. Step 0 is ceiling enforcement (edge-cases agent).
...
File: `.forge/heartbeat/token-budgets-prya.json`
...
|------|-----|----------------------|------------|
| prya | 16 GB | 2 | Never spawn opencode/kilo |
| sati | 64 GB | 6 | All types OK |
| nova | 48 GB | 4 | All types OK |
| vega | 16 GB | 2 | Never spawn opencode/kilo |
| gaea | 16 GB | 2 | Never spawn opencode/kilo |
...
**Critical finding:** prya currently shows 7 active agents despite a documented 2-agent maximum. Autoscaling on top of a system that already violates its own limits will amplify the problem. Before any autoscaling ships, the Go daemon must enforce hard per-node ceilings in code:
...
| Per-node daemon on each node | Rejected | Complexity; prya as hub is the architecture |
...
2. **Cross-node recommendations?** Phase 1 stores recommendations in prya's SQLite. sati's lead can't see them. Options:
   - (a) prya-as-hub is sufficient — sati lead checks `forge fleet recommendations` via `FORGE_API_URL=http://prya:8081`
   - (b) Write recommendations to `.forge/xnode/scale-inbox/{node}.jsonl` for local consumption
   **Recommended:** (a) — prya as hub is the existing architecture

3. **`forge fleet apply inflate` execution?** Prya can't SSH to sati for cross-node spawns. Options:
   - (a) Phase 1 scope: only spawn on the LOCAL node (orchestrator is on prya, spawns on prya only)
...
6. **Hard ceiling enforcement now or after autoscaling ships?** prya shows 7 agents vs 2-agent max. Enforce ceilings immediately (Step 0) before any autoscaling code ships?
